...
const meta = {
  title: "components/Transitions/<Transition.Scale>",
  ...
  parameters: {
    ...
    docs: {
      page: () => (
        <>
          <Title />
          <Subtitle>缩放过渡动画组件</Subtitle>

          <Markdown>
            {`
会实现 CSS 缩放的过渡动画效果，能继承使用 \`<Theme.Provider>\` 配置的全局主题化样式。

所占宽高默认取决于 \`children\` 的宽高，也可通过 \`style\` 自定义设置。

不显示时内部是通过 \`transform: scale(0); visibility: hidden\` 实现隐藏，所以仍会在 DOM 中保持挂载，并且会保留原始的宽高占位空间。
`}
          </Markdown>
...
function App() {
  ...
  return (
    <Theme.Provider>
      ...
      <Transition.Scale visible={visible}>
  ...
  return (
    <Theme.Provider>
      <Space.Flex column spacing={16}>
        ...
        <Transition.Scale visible={visible} duration={0}>
          <Card variant="outlined" style={{ width: 300 }}>
            <Typography.Text text="In 0ms" />
          </Card>
        </Transition.Scale>

        <Transition.Scale visible={visible} duration={200}>
          <Card variant="outlined" style={{ width: 300 }}>
            <Typography.Text text="In 200ms ( Default )" />
          </Card>
        </Transition.Scale>

        <Transition.Scale visible={visible} duration={1000}>
          <Card variant="outlined" style={{ width: 300 }}>
  ...
  return (
    <Theme.Provider>
      <Space.Flex column spacing={8}>
        ...
        <Transition.Scale visible={visible} onFinish={handleFinish}>
          <Card variant="outlined" style={{ width: 300 }}>
...
} satisfies Meta<typeof Transition.Scale>;
